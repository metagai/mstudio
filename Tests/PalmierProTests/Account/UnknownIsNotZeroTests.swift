...
struct UnknownIsNotZeroTests {
    ...
    @Test func theBuyButtonSurvivesAPricingOutage() throws {
        ...
        // **门槛按"报价单拉不到"那一刻的真实渲染标定。**
        //
        // 上一版写的是 `> 500`，而 500 只有长标签够得着
        // （「购买 500 credits · $9.99」）—— 那要 `creditPack != nil`，
        // 也就是**报价单没出问题**。于是这条判据只在它自称要测的那个故障
        // *不存在*时才绿；它一路绿过来，靠的是后台拉价目表的请求偶尔
        // 在上面那句断言之后、这次渲染之前落地。**竞态赢了就绿。**
        //
        // 短标签「购买 credits」实测 308。门槛放 150：
        // 画不出来是 0 上下，画出来是 300 上下，150 两边都够远。
        // **这一条问的是"有没有东西"，不是"有多少东西"。**
        #expect(lit > 150,

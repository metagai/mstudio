...
struct LocalizationTests {
    ...
    @Test func threadSafeLookupMatchesObservedLookup() async {
        ...
        let offMain = await Task.detached { L10n.threadSafe(key) }.value
        let onMain = await MainActor.run { L10n.shared(key) }
        #expect(offMain == onMain)
        #expect(offMain != key || onMain == key)
    }

    @Test func threadSafeLookupSubstitutesPlaceholders() async {
        let text = await Task.detached { L10n.threadSafe("METAG request failed (%@).", ["503"]) }.value
    ...
    @Test func threadSafeUnknownKeyFallsBackToItself() async {
        ...
        #expect(await Task.detached { L10n.threadSafe(key) }.value == key)
    }

    @Test @MainActor func unknownKeyFallsBackToItself() {
        #expect(L10n.shared("a key that does not exist") == "a key that does not exist")
    }

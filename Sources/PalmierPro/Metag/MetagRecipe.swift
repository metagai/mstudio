...
enum MetagRecipeIO {
    ...
    enum Failure: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .unrecognised: return L10n.threadSafe("That file is not a METAG recipe.")
            case .nothingToExport: return L10n.threadSafe("This timeline has no generated shots to export.")
            }
        }
    }
}
            ...
            do {
                ...
                await MainActor.run {
                    editor.mediaPanelToast = MediaPanelToast(
                        message: L10n.threadSafe("Recipe saved."), kind: .success
                    )
                }
            } catch {
                await MainActor.run {
    ...
    @objc func importRecipe(_ sender: Any?) {
        ...
        Task {
            do {
                ...
                editor.mediaPanelToast = MediaPanelToast(
                    message: L10n.threadSafe("Rebuilding %@ shots from the recipe.", [recipe.shots.count.formatted()]),
                    kind: .progress
                )

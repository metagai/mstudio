...
enum MetagReshoot {
    ...
    static func fixFlagged(asset: MediaAsset, editor: EditorViewModel) async {
        ...
        guard !staged.isEmpty else {
            editor.mediaPanelToast = MediaPanelToast(
                message: L10n.threadSafe("Nothing could be improved. Nothing was changed."),
                kind: .warning
            )
            return
        }
        editor.undo.perform(L10n.threadSafe("Fix Flagged Shots")) {
            editor.registerTimelineUndo(L10n.threadSafe("Fix Flagged Shots")) { vm in
        ...
        editor.mediaPanelToast = MediaPanelToast(
            message: L10n.threadSafe("Fixed %@ shots.", [staged.count.formatted()]), kind: .success
        )
    }
    ...
    private static func adoptNewTake(
        ...
        do {
            ...
            editor.undo.perform(L10n.threadSafe("Re-shoot")) {
                editor.registerTimelineUndo(L10n.threadSafe("Re-shoot")) { vm in
    ...
    private static func message(for error: Error) -> String {
        if case MetagGateway.Failure.http(402) = error {
            return L10n.threadSafe("Re-shooting needs a subscription, and works only on shots made with our own engine.")
        }
        if case MetagGateway.Failure.http(429) = error {
            return L10n.threadSafe("Hourly limit reached. Try again later.")
        }
        return L10n.threadSafe("The re-shoot could not start.")
    }
}

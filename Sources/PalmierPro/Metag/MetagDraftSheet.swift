...
final class MetagDraftModel: ObservableObject {
    ...
    var stageText: String {
        switch job?.stage {
        case "storyboard": return L("Writing the shot list…")
        case "voice": return L("Choosing a narrator voice…")
        case "narration": return L("Recording the narration…")
        case "frames": return L("Painting the first frames…")
        case "music": return L("Scoring it…")
        default:
            // 阶段还没上来（刚提交、或老网关）。**不给数字** ——
            // 原来那句写着"约 40 秒"而实测是 53–97 秒，给错的数字比不给更糟。
            return frames.isEmpty
                ? L("Drafting… this one is free")
                : L("Frames are landing, still writing the narration…")
        }
    }
    ...
    private func poll(_ id: String) async {
        ...
        note = L10n.threadSafe("The draft is taking too long — try again in a moment.")
    }
}
...
struct MetagDraftSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.md) {
            Text(L("See the draft first, then decide")).font(.headline)
            if model.jobId == nil {
                promptStage
            } else if model.ready {
                draftStage
    ...
    private var promptStage: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.sm) {
            ...
            HStack {
                ...
                Button(L("Draft it")) { Task { await model.draft() } }
        ...
        VStack(alignment: .leading, spacing: AppTheme.Spacing.sm) {
            ...
            if engine != "local" {
                ...
                if !allShots {
                    ...
                    Text(L("By default only lip-sync shots use it; the rest stay on the standard tier"))
                        .font(.caption2).foregroundStyle(.secondary)
                }
            }
            HStack {
                    ...
                    Task {
                        if let job = await model.approve(engine: engine, allShots: allShots) {
                            editor.mediaPanelToast = MediaPanelToast(
                                message: L10n.threadSafe("Generating — shots will land as they finish."),
                                kind: .progress)

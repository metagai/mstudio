...
struct MetagDirectorSheet: View {
    ...
    private var header: some View {
        HStack(spacing: AppTheme.Spacing.sm) {
            ...
            Text(L("Auto Director"))
                .font(.system(size: AppTheme.FontSize.lg, weight: AppTheme.FontWeight.semibold))
                .foregroundStyle(AppTheme.Text.primaryColor)
            Spacer(minLength: 0)
            if account.isSignedIn {
                Text(L("%@ credits", account.remainingCredits.formatted()))
                    .font(.system(size: AppTheme.FontSize.xs))
                    .monospacedDigit()
    ...
    private var setupBody: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.md) {
            ...
            TextField("What should it be about?", text: $topic, axis: .vertical)
                ...
                .lineLimit(3...3)
                .font(.system(size: AppTheme.FontSize.sm))
            Text(L("Every paid step quotes first. Nothing is billed until you approve."))
                .font(.system(size: AppTheme.FontSize.xs))
                .foregroundStyle(AppTheme.Text.tertiaryColor)
        }
    }
        ...
        return Button {
            ...
            VStack(alignment: .leading, spacing: AppTheme.Spacing.xxs) {
                ...
                Text(item.isSupported ? item.tagline : "Available in METAG Studio only")
                    ...
                    .foregroundStyle(AppTheme.Text.tertiaryColor)
                    .lineLimit(2)
                Text(L("~%@s · %@ credits · ~%@ min", Int(item.target_seconds).formatted(), item.estimated_cost_credits.formatted(), max(1, item.estimated_wait_seconds / 60).formatted()))
                    .font(.system(size: AppTheme.FontSize.xxs))
                    .foregroundStyle(AppTheme.Accent.primary)
            }
    ...
    private func runBody(_ run: MetagDirector.Run) -> some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.md) {
            ...
            if run.status == "done" {
                Text(L("Done. Shots are landing in the media library."))
                    .font(.system(size: AppTheme.FontSize.xs))
    ...
    private func budgetBar(_ run: MetagDirector.Run) -> some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.xxs) {
            HStack {
                Text(L("Budget"))
                Spacer(minLength: 0)
                Text(L("%@ / %@ credits", run.spent_credits.formatted(), run.budget_credits.formatted())).monospacedDigit()
            }
            .font(.system(size: AppTheme.FontSize.xxs))
            .foregroundStyle(AppTheme.Text.tertiaryColor)
    ...
    private func quoteCard(_ quote: MetagDirector.Run.Quote) -> some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.xxs) {
            ...
            Text(L("%@ costs %@ credits — %@, %@ shots. Billed only after you approve.", quote.stage.capitalized, quote.cost_credits.formatted(), quote.engine, quote.shots.formatted()))
    ...
    private var footer: some View {
        HStack(spacing: AppTheme.Spacing.sm) {
            if let run, !run.isTerminal {
                Button(L("Stop")) { act { try await MetagGateway.cancelDirectorRun(run.id) } }
                    .buttonStyle(.plain)
                    .focusable(false)
                    .disabled(busy)
            }
            ...
            Button(L("Close")) { isPresented = false }
                ...
                .focusable(false)
            if run == nil {
                Button(account.isSignedIn ? L("Start") : L("Sign In to Start"), action: start)
                    .buttonStyle(.editorPrimary)
                    .focusable(false)
                    .disabled(account.isSignedIn ? !canStart : busy)
            }
            if let run, run.isAwaitingApproval {
                Button(L("Approve %@ Credits", (run.artifacts.quote?.cost_credits ?? 0).formatted())) {

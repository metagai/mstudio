...
enum MetagGateway {
    ...
    struct Account: Decodable, Sendable {
        ...
        let sub: String
        let credits: Int
        /// **这个账号从没出过片。** 网关 a728ad8 起回这个键（口径：草案不算 ——
        /// 草案免费，看过一条草案就走的人下次回来仍然是"第一次"）。
        ///
        /// 客户端据此把**第一条片子的默认档**设成会动的那一档而不是自研档。
        /// 依据是实测：同一段提示词，自研档平均 motion 5.66（30 天 461 镜）、
        /// wan-flash 16.73，而第一条片子决定他还回不回来。
        ///
        /// ⚠ **服务端不升档，也不该升。** 它只回这一个事实，默认档是客户端设的、
        /// 价格照旧写在按钮上 —— 服务端偷偷把 local 换成 wan-flash 会让实扣
        /// 高于他看到的报价，那和我们这两天清掉的那一族同性质。
        ///
        /// ⚠ **取不到时按 false 处理**（网关自己也是这么定的）：数据库抖一下的时候，
        /// 错的方向必须是"少给他一次中档"，不是"多花他的钱"。
        var first_film: Bool? = nil
    ...
    struct Pricing: Decodable, Sendable {
        ...
        struct Engine: Decodable, Sendable, Identifiable {
            ...
            /// 这一档**能不能免费试渲一镜**。登记表的一列，由网关带出来。
            ///
            /// 为什么要这一列：客户端此前用"最便宜的付费档"这个**代理量**
            /// 去推断"能试渲的档"，而服务端那侧是一份手写白名单。
            /// 2026-09-20 查实：线上最便宜的付费档是 wan-flash（7cr，
            /// 界面上写着 "Best value 720P"），它不在白名单里 ——
            /// **于是那颗按钮对每一个停留在默认档位的人都必然 400**，
            /// 而线上"使用 0 次"被我们读成了"没人要"。
            ///
            /// 老网关不回这个字段时是 nil。**nil 不当 false 用** ——
            /// 见 `sampleTier`：整份报价单一个都不带这个字段，就说明
            /// 服务端还没上这一列，那时按老规则挑档（行为不变），
            /// 而不是让这颗按钮从所有人的界面上静默消失。
            var sampleable: Bool? = nil
    ...
    struct Job: Decodable, Sendable {
        ...
        /// 分镜边写边露的那几句旁白。
        ///
        /// **网关的判据里点名的就是我们**：`delivery.rs` 那条
        /// 「Redis 里有 storyboard_preview，REST 就必须交出来」写着
        /// 「macOS 端（只轮询 REST、不连 WS）的用户在那 60 多秒里一个字都看不到」。
        /// **已接**（`MetagDraftSheet.applyStreamed`）—— 这一行原来是现在时
        /// 「Mac 一直没解这个字段」，而它 2026-09-07 已经不成立了：
        /// 我按这句话去找"还缺什么"，白查了一轮。**注释叙述历史要用过去时，
        /// 并写清现在在哪儿做的** —— 否则它就是下一个人的假线索。
        ///
        ...
        ///
        /// ⚠ **别引用"约 2 秒"那个数。** studio 那侧的注释写着「约 2 秒就到」，
        /// 而合伙人 09-07 真站掐表：**按下 → 它上屏 9351ms**。
        /// 那个 2 秒的主语是 **worker 写出它的时刻**，不是用户看见它的时刻；
        /// 9.4 秒里最大一块是"按下到 preview 请求发出"（1.4–3.8s，
        /// 浏览器在下编辑器分块和 ffmpeg 的 wasm，preview 排在它们后面）。
        /// **服务端没问题** —— 第一次轮询回来时它就已经在响应里了。
        ///
        /// Mac 这条路没有路由切换和懒加载，**但我没量过，所以这里不写数**
        /// （`draft_seen.press_to_request_ms` 已经在记，等第一个真人）。
        /// 它仍然是这段等待里最早到的、属于他那部片子的东西 —— 那一点不取决于秒数。
        ///
        ...
        ///
        /// 网关一直在发、web 的幕布一直在播，而 Mac **曾经**从来没解过它 ——
        /// 于是"先看一眼"给的是一排静态图加一堆输入框：那不是"看一眼"，
        /// 是"看一眼它的证据"。**已接**：`MetagDraftSheet.draftStage`
        /// 拿到 `preview` 就换成 `MetagDraftPlayer`，拿不到才退回场记板。
        let preview: String?
        /// 失败的**种类**，不只是失败的那句话。
        ///
        ...
        ///
        /// Mac **曾经**没解过它，于是那个"据此决定"的界面从来没拿到过依据：
        ...
        ///
        /// Mac **曾经**没下过它（已接：`MetagJobOpener`）。那时用户看的是一条有配乐的草案，
        /// 批准之后拿到的时间线上只有画面和旁白 ——
        ...
        /// 旁白比镜头短，一段一段挨着铺会越走越偏。
        /// **第一张画面出现在世界上的那一刻**（毫秒时间戳，还没出现时是 nil）。
        ///
        /// 产品技术负责人为这件事加的：网关那侧首帧约第 13 秒就绪，而 Mac
        /// 轮询一轮实际 5.2 秒（4s 睡 + 从国内打过去实测 1.0–1.5s 的来回），
        /// 于是他**第 15.6 秒才问到、第 17.4 秒才看见**。
        ///
        /// 中间那 4.4 秒没有任何一方负责，**也没有任何一处会红** ——
        /// 有了这个时间戳，它第一次成为一个能量的数。
        ///
        /// ⚠ 是毫秒，不是秒。
        ...
        /// **草案和时间线之间那一档响度差**，每一单实测出来的（不是常数）。
        ///
        /// `preview.mp4` 最后过了一道 `loudnorm=I=-16`，而时间线是
        /// N 段 + 配乐床直接相加、没过 —— 网关那侧实测整条低约 5.8 dB。
        ///
        /// **不读它不等于"中性"，等于"偏 6 dB 且没人补"**：
        /// 他刚看完草案、点"进编辑器"，同一部片子突然轻了一半
        /// （6 dB 约等于感知响度减半），导出去也比各平台的 -14 目标低一截。
        /// 他不会报这个 bug，他会觉得"这软件声音怪怪的"。
        let master_gain_db: Double?

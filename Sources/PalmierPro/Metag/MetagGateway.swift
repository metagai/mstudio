...
enum MetagGateway {
    ...
    enum Failure: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .http(404):
                ...
                // 说"已过期"对拿到别人链接的人是假话。
                return L10n.threadSafe("That job is gone — jobs are kept for 24 hours, free files for 60 minutes.")
            case .http(409):
                return L10n.threadSafe("Not ready — the draft is still rendering, or this one was already produced.")
            case .http(413):
                return L10n.threadSafe("That file is too large — images up to 8 MB, voice samples up to 16 MB.")
            case .http(415):
                return L10n.threadSafe("That file type is not supported — images as PNG/JPEG/WebP, audio as WAV/MP3/M4A.")
            case .http(403):
                return L10n.threadSafe("That is not yours to open.")
            case .http(502), .http(504):
                return L10n.threadSafe("The provider did not answer — try again in a moment.")
    ...
    static func currentLanguageCode() async -> String {
        await MainActor.run { L10n.shared.gatewayLanguageCode }
    }

...
extension TimelineView {
    ...
    func reshootSubmenu(for clipId: String) -> NSMenu? {
        ...
        let entries: [(String, Bool, Int)] = [
            (L10n.threadSafe("Re-shoot This Shot"), false, 1),
            (L10n.threadSafe("Another Composition"), true, 1),
            (L10n.threadSafe("Three More Takes"), true, 3),
        ]
        ...
        let fix = NSMenuItem(title: L10n.threadSafe("Fix Flagged Shots"), action: #selector(performFixFlagged(_:)), keyEquivalent: "")
        fix.target = self
        fix.representedObject = clipId
        fix.toolTip = L10n.threadSafe("Re-runs only shots the automatic check flagged. What you have is kept as a take.")
        menu.addItem(fix)
        return menu
    }

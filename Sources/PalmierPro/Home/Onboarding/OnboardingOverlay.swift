...
struct OnboardingOverlay: View {
    ...
    /// 「先看一眼你的片子」按下去发生什么。
    ///
    /// **它此前多发了一条没人听的通知。** `.metagStartDraft` 只有编辑器里的
    /// 素材面板在听（`MediaTab`），而首次打开时编辑器还不存在 ——
    /// 于是这颗按钮除了关掉引导层，什么都没做。
    ///
    /// 加它的那次提交（`f46e8489`，8/31）的信息写着
    /// **「1112 个人落地，0 个人打过一行字」** —— 它就是为了修这件事，
    /// 而它自己的主按钮是空转的。**修 A 的那一手本身也坏了，而没人会去验它。**
    ///
    /// ⚠ 它还不只是"没用"：多窗口时后台可能真有一个编辑器在听，
    /// 那条通知会在他没看的那扇窗里弹出一张草案卡。
    ///
    /// 现在只做一件事：关掉引导，把他放到首屏 ——
    /// **那里就是写那一句的地方**（0.1.14 之后第一屏本身就是产品）。
    /// 焦点由 `HomeHero` 在引导结束时接手。

...
final class SkillCatalog {
    ...
    /// Tries each source in order (mirror, then upstream) and keeps the first catalog that
    /// both downloads and decodes. Only if every source fails is an error surfaced; the
    /// on-disk cache from a previous run stays intact in that case.
    func refresh() async {

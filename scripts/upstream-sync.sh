...
4f7ecfa|taken|已拿（1f7bcb8b）：关掉 What's New 之后首页点不动 —— 不可见的 GeometryReader 把点击全吃了。我们树上是一模一样的形状。冲突只是底色（我们用 Color.black，上游换了主题色），保留我们的。**没有自动化判据**：pending 是 private(set)，测试里造不出那个状态；仓库里也没有 ViewInspector/XCUITest，SwiftUI 的命中测试够不着。这一条是照抄上游的修复，未经我们自己验证
